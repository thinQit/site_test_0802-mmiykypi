...
export default function PricingTable({
  ...
  subheadline = 'Flexible collections designed for intimate stories and bold campaigns.',
  tiers = [],
        ...
        <div className="mt-16 grid gap-8 md:grid-cols-3">
          {tiers.map(function (tier, i) {
            return (
              <Card
                key={i}
                className={'relative flex flex-col rounded-xl border bg-card shadow-sm transition-shadow hover:shadow-md ' + (tier.highlighted ? 'border-primary ring-2 ring-primary scale-105' : 'border-border')}
              >
                {tier.highlighted && (
                ...
                <CardHeader className="text-center">
                  ...
                  <CardDescription className="font-light">{tier.description}</CardDescription>
                  <div className="mt-4">
                ...
                <CardFooter>
                  <Button className="w-full rounded-lg px-6 py-3 font-medium tracking-tight" variant={tier.highlighted ? 'default' : 'outline'} asChild>
